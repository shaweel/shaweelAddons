...
function tyfr() {
	...
	utils.serverSchedule(15, () => {
		ChatLib.command("p leave")
	});
}


let shitters = FileLib.read("shaweelAddons", "config/shitters.json")

if (shitters === null) {
	FileLib.write("shaweelAddons", "config/shitters.json", [])
...
function saveShitters() {
	...
	FileLib.write("shaweelAddons", "config/shitters.json", toSave)
}
...
function shitter(arg2, arg3, reason) {
	...
	if (arg2 == "reload") {
		shitters = FileLib.read("shaweelAddons", "config/shitters.json")
		shitters = JSON.parse(shitters)
...
register("command", (...args) => {
	...
	if (args[0] == "help") {
		...
		ChatLib.chat("&a/sha resetrattracker&7 - Resets the Hub Rat Profit Tracker")
	...
	if (args[0] == "resetrattracker") {
		ChatLib.command("sharesetrattracker", true)
		return
	}
