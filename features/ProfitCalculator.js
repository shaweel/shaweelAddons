...
import { elements, editGui, positions, setShouldRender, clearLines, getLines, setLine } from "./Gui.js"
...
function getProfit(name) {
    ...
    if (name.includes("Thorn")) {
        profit = bz.SHARD_THORN.quick_status.sellPrice
    }
    if (name.includes("Scarf")) {
        profit = bz.SHARD_SCARF.quick_status.sellPrice
    }
...
register("tick", () => {
    ...
    if (inChest()) {
        ...
        if (chest == "Wood Chest") {
            setLine(2, 0, "&fWood Chest")
        } else if (chest == "Gold Chest") {
            setLine(2, 0, "&6Gold Chest")
        } else if (chest == "Diamond Chest") {
            setLine(2, 0, "&bDiamond Chest")
        } else if (chest == "Emerald Chest") {
            setLine(2, 0, "&2Emerald Chest")
        } else if (chest == "Obsidian Chest") {
            setLine(2, 0, "&5Obsidian Chest")
        } else if (chest == "Bedrock Chest") {
            setLine(2, 0, "&8Bedrock Chest")
        }
        let index = 0
        let totalprofit = 0
    ...
    if (inChestMenu()) {
        ...
        for (let chest of chests) {
            ...
            if (profit > 0) {
                ...
                setLine(2, index, chestName+"&7 - &e0")
            } else {
                setLine(2, index, chestName+"&7 -&c "+formatted)
            }
...
register("guiRender", () => {
    if (editGui.isOpen()) return
    setShouldRender(2, false)
    let index = -1
    for (let line of getLines(2)) {
        index++
        try {
