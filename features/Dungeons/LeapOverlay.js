...
register("guiOpened", (event) =>  {
	if (!Settings.leapOverlay) return
	let localSlots = event.gui.field_147002_h//inventorySlots
...
let savedKeybinds = FileLib.read("shaweelAddons", "config/leapKeybindsBackup.json")

if (savedKeybinds === null) {
	FileLib.write("shaweelAddons", "config/leapKeybindsBackup.json", JSON.stringify({"Archer": 0, "Berserk": 0, "Healer": 0, "Mage": 0, "Tank": 0}))
...
register("tick", () => {
	...
	FileLib.write("shaweelAddons", "config/leapKeybindsBackup.json", JSON.stringify(savedKeybinds))
})

register("guiKey", (char, keyCode, gui, event) => {
	if (!Settings.leapOverlay || !Settings.leapKeybinds) return
	if (!leapGui.isOpen()) return
	if (keyCode == 18 || keyCode == 1) {
		leapGui.close()
	}
	event.setCanceled(true)
	let lines = TabList.getNames()
	...
	if (keyCode == mageKeybind.getKeyCode() && mage !== null) {
		leapTo(mage, "Mage")
	}
	if (keyCode == archerKeybind.getKeyCode() && arch !== null) {
		leapTo(arch, "Archer")
	}
	if (keyCode == healerKeybind.getKeyCode() && heal !== null) {
		leapTo(heal, "Healer")
	}
	if (keyCode == tankKeybind.getKeyCode() && tank !== null) {
		leapTo(tank, "Tank")
	}
	if (keyCode == berserkKeybind.getKeyCode() && bers !== null) {
		leapTo(bers, "Berserk")
	}
})